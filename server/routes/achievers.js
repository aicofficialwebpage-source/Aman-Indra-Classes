...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error retrieving achievers.', error: error.message });
  }
});
...
router.post('/', auth, upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Error creating achiever profile.', error: error.message });
  }
});
...
router.put('/:id', auth, upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Error updating achiever.', error: error.message });
  }
});
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error deleting achiever.', error: error.message });
  }
});
